...
export default function PublicGuruDetailPage() {
  ...
  return (
    <PublicLayout unit={unit}>
      <div className="pt-8 pb-32 bg-[#C8F7F5] min-h-screen">
        <div className="max-w-4xl mx-auto px-6">
          
          {/* Breadcrumb navigasi */}
          <nav className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider mb-8" style={{ color: "#0B6B69" }}>
            <Link href={`/${unit}`} className="hover:opacity-85 transition-opacity">
              Beranda
            </Link>
            <ChevronRight size={12} style={{ color: "#0FA8A4" }} />
            <Link href={`/${unit}/guru`} className="hover:opacity-85 transition-opacity">
              Guru
            </Link>
            <ChevronRight size={12} style={{ color: "#0FA8A4" }} />
            <span style={{ color: "#0B6B69" }} className="truncate max-w-[200px]">
          ...
          <button 
            ...
            onMouseLeave={(e) => {
              ...
              e.currentTarget.style.color = "#0B6B69";
          ...
          {isLoading ? (
            <div className="flex justify-center py-20 bg-white rounded-[40px] border border-[#7EE6E3] shadow-sm">
              <div className="animate-spin rounded-full h-12 w-12 border-t-4 border-[#0FA8A4] border-gray-100"></div>
            </div>
          ) : error ? (
            // Penanganan Kesalahan jika data guru tidak ditemukan
            <div className="bg-white p-12 text-center rounded-[40px] border border-[#7EE6E3] shadow-lg">
              <User size={48} style={{ color: "#0FA8A4", margin: "0 auto 20px" }} />
              <h2 className="text-2xl font-black mb-4" style={{ color: "#0B6B69" }}>Profil Tidak Tersedia</h2>
              <p className="font-medium mb-8" style={{ color: "#0FA8A4" }}>{error}</p>
              <button 
                onClick={handleGoBack}
                className="px-8 py-4 rounded-xl font-black uppercase text-xs tracking-widest transition-all"
                style={{ backgroundColor: "#2FCFC9", color: "#0B6B69" }}
              >
            ...
            guru && (
              ...
              >
                {/* Bagian Foto Profil */}
                <div className="w-full md:w-[360px] flex-shrink-0 aspect-[3/4] md:aspect-auto md:min-h-[480px] bg-[#C8F7F5] overflow-hidden relative border-b md:border-b-0 md:border-r" style={{ borderColor: "#7EE6E3" }}>
                  <img
                    src={
                      guru.foto ||
                ...
                <div className="p-8 md:p-12 flex flex-col justify-center flex-1">
                  ...
                  {/* Jabatan */}
                  <span className="text-xs font-black uppercase tracking-[0.2em] mb-3 block" style={{ color: "#0FA8A4" }}>
                    {guru.jabatan || "Tenaga Pendidik"}
                  </span>
                  
                  {/* Nama */}
                  <h1 className="text-3xl md:text-4xl font-black leading-tight mb-8" style={{ color: "#0B6B69" }}>
                    {guru.nama}
                  </h1>

                  <div className="space-y-6">
                    {/* NIP */}
                    <div className="flex items-start gap-4 p-4 rounded-2xl" style={{ backgroundColor: "#C8F7F5" }}>
                      <Award size={20} style={{ color: "#0B6B69", marginTop: "2px" }} className="shrink-0" />
                      <div>
                        <span className="text-[10px] font-black uppercase tracking-widest block mb-1" style={{ color: "#0FA8A4" }}>
                          Nomor Induk Pegawai (NIP)
                        </span>
                        <span className="font-bold text-sm" style={{ color: "#0B6B69" }}>
                    ...
                    {/* Mata Pelajaran */}
                    <div className="flex items-start gap-4 p-4 rounded-2xl" style={{ backgroundColor: "#C8F7F5" }}>
                      <BookOpen size={20} style={{ color: "#0B6B69", marginTop: "2px" }} className="shrink-0" />
                      <div>
                        <span className="text-[10px] font-black uppercase tracking-widest block mb-1" style={{ color: "#0FA8A4" }}>
                  ...
                  <div className="mt-10 pt-8 border-t" style={{ borderColor: "#C8F7F5" }}>
                    <h3 className="text-xs font-black uppercase tracking-[0.25em] mb-5" style={{ color: "#0FA8A4" }}>
                      Hubungi Guru
                    </h3>
                    ...
                    <div className="flex flex-wrap gap-4">
                      ...
                      {guru.whatsapp ? (
                        <a
                          href={`https://wa.me/${guru.whatsapp}`}
                          target="_blank"
                        ...
                        <span className="text-xs font-bold py-3.5 px-5 rounded-2xl" style={{ backgroundColor: "#C8F7F5", color: "#0B6B69" }}>
                      ...
                      {(guru.gmail || guru.email) ? (
                        <a
                          ...
                          onMouseEnter={(e) => {
                            e.currentTarget.style.backgroundColor = "#C8F7F5";
                            e.currentTarget.style.transform = "translateY(-2px)";
                          }}
                          onMouseLeave={(e) => {
