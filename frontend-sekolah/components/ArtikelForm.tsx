...
export default function ArtikelForm({ unit }: { unit: "sd" | "smp" }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (thumbnail) {
        ...
        formData.append("path", `artikel/${unit}`);
      ...
      router.push(`/admin/${unit}/artikel`);
    } catch (error: any) {
      const msg = error.response?.data?.message || "Gagal menambahkan artikel.";
      toast.error(msg);
      console.error(error);
    } finally {
      setIsLoading(false);
  ...
  const themeBtn = "bg-tosca-500 hover:bg-tosca-700 shadow-tosca-500/30";
    ...
    <form onSubmit={handleSubmit} className="bg-white p-6 md:p-10 rounded-[32px] md:rounded-[40px] shadow-sm border border-gray-100 space-y-6 md:space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10">
        <div className="lg:col-span-2 space-y-6 md:space-y-8">
          <div>
            <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-2 md:mb-3">Judul Artikel</label>
            <input
              type="text"
              value={judul}
          ...
          <div>
            <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-2 md:mb-3">Tanggal</label>
            <input
              type="date"
              value={tanggal}
          ...
          <div>
            <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-2 md:mb-3">Isi Artikel</label>
            <textarea
              value={konten}
        ...
        <div className="space-y-6 md:space-y-8">
          <div>
            <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-2 md:mb-3">Upload Gambar</label>
               ...
               <div className={`aspect-video rounded-2xl md:rounded-3xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center overflow-hidden bg-gray-50/50 group-hover:border-tosca-500 transition-all ${preview ? 'border-solid' : ''}`}>
                  {preview ? (
                    ...
                    <div className="text-center p-4 md:p-6 flex flex-col items-center gap-2 md:gap-3">
                       ...
                       <p className="text-[10px] md:text-xs font-bold text-gray-400">Pilih gambar</p>
                    </div>
                  )}
                  <input
                    type="file"
