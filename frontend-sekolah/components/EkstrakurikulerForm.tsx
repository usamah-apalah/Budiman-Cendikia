...
export default function EkstrakurikulerForm({ unit }: { unit: "sd" | "smp" }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (image) {
        ...
        uploadData.append("path", `ekskul/${unit}`);
      ...
      router.push(`/admin/${unit}/ekstrakurikuler`);
    } catch {
    ...
    <form onSubmit={handleSubmit} className="bg-white p-6 md:p-10 rounded-[32px] md:rounded-[40px] shadow-sm border border-gray-100 space-y-6 md:space-y-8">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10">
        <div className="lg:col-span-2 space-y-4 md:space-y-6">
          <div>
            <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Nama Ekstrakurikuler</label>
            <input 
              type="text" 
              value={nama} 
              onChange={e => setNama(e.target.value)} 
              required 
              className="w-full px-4 py-3 md:px-5 md:py-3 rounded-xl md:rounded-2xl border border-gray-200 focus:ring-4 focus:ring-tosca-500/10 focus:border-tosca-500 outline-none font-bold" 
              placeholder="Nama ekskul..." 
            />
          </div>
          <div>
            <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Jadwal Pelaksanaan</label>
            <input 
              type="text" 
              value={jadwal} 
              ...
              required 
              className="w-full px-4 py-3 md:px-5 md:py-3 rounded-xl md:rounded-2xl border border-gray-200 focus:ring-4 focus:ring-tosca-500/10 focus:border-tosca-500 outline-none font-bold" 
              placeholder="Contoh: Rabu, 14.00 - 15.30" 
            />
          </div>
          <div>
            <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Deskripsi Kegiatan</label>
            <textarea 
              value={deskripsi} 
              onChange={e => setDeskripsi(e.target.value)} 
              rows={6} 
              required 
              className="w-full px-4 py-3 md:px-5 md:py-3 rounded-xl md:rounded-2xl border border-gray-200 focus:ring-4 focus:ring-tosca-500/10 focus:border-tosca-500 outline-none leading-relaxed" 
              placeholder="Tuliskan penjelasan mengenai kegiatan ekstrakurikuler..."
            ></textarea>
          </div>
        </div>
        ...
        <div className="space-y-6 md:space-y-8">
          <div>
            <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-2 md:mb-3">Gambar / Ikon Lampiran</label>
      ...
      <div className="flex flex-col-reverse sm:flex-row justify-end gap-3 md:gap-4 pt-6 md:pt-8 border-t border-gray-100">
        ...
        <button type="submit" disabled={isLoading} className={`w-full sm:w-auto px-8 py-3 md:px-12 md:py-4 rounded-xl md:rounded-2xl text-white font-black text-[10px] md:text-xs uppercase tracking-widest transition-all flex items-center justify-center shadow-lg ${themeBtn} ${isLoading ? 'opacity-70 cursor-not-allowed scale-95' : 'hover:-translate-y-1'}`}>
          {isLoading ? 'Menyimpan...' : 'Simpan Ekstrakurikuler'}
        </button>
      </div>
    </form>
  );
